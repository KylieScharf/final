...
In search of an answer to our question, we decided to use data sourced from Gap minder to investigate a possible relationship. We used two data sets, one containing information of the average amount of babies per woman per country and year, and one containing average life expectancy per country per year. In order to create the data sets we obtained, Gap Minder had different ways of collecting the data over time. For the Babies per woman data set, between 1800 to 1950 the Gap Minder Historic estimate was used which was was compiled and documented by Mattias Lindgren. Between 1950 and 2021, Gap Minder used estimates of the UN World Population Prospect 2022 to obtain the Total Fertility Rate ("Babies per Woman"). For the Life Expectancy data, Gap Minder used three separate sources and combined them into one. For the years 1800 to 1970, Gap Minder used data they collected that is based on 100 sources compiled by Klara Johnson and Mattias Lindgren. For the years 1970 to 2016, Gap Minder used the Global Burden of Disease Study 2017. This data set only contains data for 1990 to 2017, so from 1970 to 1990 Gap Minder used data from the Global Burden of Disease Study 2016 from the Institute for Health Metrics and Evaluation (IHME) of the University of Washington, Seattle ("Life Expectancy at Birth"). 

To clean the data, we decided to first subset the two original large data sets imported to only contain observations from the years 1900 to 2000. We did this to limit the number of observations to be easier to work with and because we are only really interested in these years to base our conclusion on. We also decided to create a new variable called decade so that we may average over this in the future to reduce the amount of output. We then decided to use an inner join to join our data because we only wanted to include observations with an observed value within both data sets. Each observational unit in our joined data represents one specific country in one specific year and contains the values of both variables. Along with a variable for country, our final cleaned data set also contains a variable for year, life expectancy, babies per woman, and decade. In total, our final data set contains 18,786 observations from 186 countries and from the years 1900 to 2000.

We hypothesized a negative relationship between average babies per year per country and average life expectancy per year per country. That is, we expect average life expectancy to decrease as the average amount of babies per woman increases. We believe this because as the average babies per woman increases, the population increases which creates more competition for limited resources. This is likely to lead to earlier deaths, and an increased number of deaths due to factors other than just natural causes. Our motivation for selecting this topic stems from a blend of personal and societal interests. Firstly, as individuals contemplating starting families, we're naturally curious about the potential impacts of fertility rates on life expectancy. Understanding these dynamics can inform our personal decisions and contribute to our collective understanding of family planning.
...
In order to test our hypothesized relationship, we needed to fit a model to our data. The method we decided on was a linear regression model. Before we could fit such a model, however, we needed to check that all assumptions of the model were met by our data. Below are graphs used to check some of the assumptions. 
...
As seen in the plots above, the simulated average life expectancy data appears to be relatively similar to the observed average life expectancy data. They seem to both have similar ranges and have very similar linear trends. The observed data appears to be more spread out than the simulated but not by much. This shows that a linear model appears to be a very good fit but not perfect. 
